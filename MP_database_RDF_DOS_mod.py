...
def get_element(material):
    """
    
    """
    compos = Composition(material['pretty_formula']).as_dict()
...
# Advance filtering: data with elements in spd_element only.
spd_element = ["Be", "Li", "Na", "Mg", "Al", "K", "Ca", "Sc", "Ti", "V", "Cr", "Mn", "Fe", "Co", "Ni", "Cu", "Zn", "Ga","Rb", "Sr", "Y", "Zr", "Nb", "Mo", "Tc", "Ru", "Rh", "Pd", "Ag", "Cd", "In", "Sn"]

for data in datas:
    compos = Composition(data['pretty_formula']).as_dict()
    compos = list(compos.keys())
    if len(compos) == 1:
        if Element[compos[0]].symbol in spd_element:
        ...
        if Element[compos[0]].symbol in spd_element:
            if Element[compos[1]].symbol in spd_element:
                formula.append(data['pretty_formula'] + ' ' + data['material_id'])
                ids.append(data['material_id'])
            else:
                pass
        ...
        if Element[compos[0]].symbol in spd_element:
            if Element[compos[1]].symbol in spd_element:
                if Element[compos[2]].symbol in sp_delement:
                    formula.append(data['pretty_formula'] + ' ' + data['material_id'])
                    ids.append(data['material_id'])
                else:
                    pass
...
for i, identity in enumerate(ids[:8]):
    try:
        # Mining DOS
